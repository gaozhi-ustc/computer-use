...
Endpoints:
    POST /frames/upload  — ingest one frame (multipart, Phase 1+)
    GET  /frames         — query by employee_id / session_id
    GET  /health         — liveness probe

Authentication:
    If env WORKFLOW_SERVER_KEY is set, every upload request requires
    the same value in an `X-API-Key` header. If the env var is unset,
    auth is disabled — handy for local dev.
